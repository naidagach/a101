...
function ProductSwiper({title, linkTo, imgUrl, data}) {
  return (
    <>
      <div className='flex justify-between items-center py-4'>
        <h3 className='font-medium text-base'>{title}</h3>
        <a className='flex items-center text-sm text-brand-blue-primary gap-1 font-medium' href={linkTo}>
          Tümünü Gör <FaChevronRight />
        </a>
      </div>
      <section className='flex max-h-[332px] gap-2'>
        <div className='min-w-[200px] hidden m:block rounded-2xl overflow-hidden'>
          <img className='size-full' src={imgUrl} alt={title} />
        </div>
        <Swiper
          slidesPerView={2.1}
          spaceBetween={10}
          ...
          navigation={true}
          modules={[Navigation]}
          className="mySwiper py-[20px] max-w-[1200px] mx-auto "
        >
            <div>
              <img src="https://cdn2.a101.com.tr/dbmk89vnr/CALL/Image/getw/files/686574e8cb86a500078bafe4/reco.png?width=164" alt="" />
            </div>
          {data && data.map(item => {
            return (
            <SwiperSlide>
                ...
                <div className="flex product-container bg-white flex-col relative p-2 w-full border border-[#e5e7eb] rounded-2xl">
                  ...
                  <div className="left-3 right-3 product-add-button ">
                    <button
                      ...
                      className="bg-brand-blue-primary rounded-full p-1 text-sm text-center w-full text-white undefined"
                    >
                      Sepete Ekle
                    </button>
                  </div>
